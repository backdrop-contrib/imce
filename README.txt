...
- Image(jpg, png, gif) operations: resize, thumbnails, preview
...
- User-friendly interface: directory and file listing, file sorting(by name, size, dimensions, date), selecting, preview.
- Built-in support for inline image/file insertion into textareas.
New in 6.x:
- Improved UI.
- Themable layout using tpl files.
- Multiple file selection and operation.
...
Probably talking about tinyMCE or FCKeditor. See INTEGRATION METHODS to learn to integrate by yourself.
...
FCKeditor: It's sufficient to add these lines into fckeditor profile->advanced settings->custom javascript configuration;
...
FlashBrowserURL= '/?q=imce&app=FCKEditor|url@txtUrl';//make sure the base URLs are correct
...
Now we will define the openFileBrowser function that opens IMCE and makes it fill our url field on file selection.


INTEGRATION BY URL

When IMCE is opened using an url that contains &app=appName|fileProperty1@FieldId1|fileProperty2@FieldId2|...
...
In our CASE, we should open IMCE using this URL: /?q=imce&app=myApp|url@urlField
...
INTEGRATION BY imceOnLoad AND setSendTo

A more flexible method that gives a finer control over IMCE.

var imcePopup;// this is our global variable referencing to IMCE window.
...
function openFileBrowser() {
  ...
  if (typeof imcePopup == 'undefined' || imcePopup.closed) {
    ...
    //we make a function to run when IMCE loads, by setting the imceOnLoad property of the window
    //It is automatically called by IMCE with a single parameter(win) referencing to IMCE window.
    //We can access all IMCE methods using win.imce
    imcePopup['imceOnLoad'] = function (win) {
      //we use imce.setSendTo method to make IMCE send selected files to our imceFinish function
...
//file is the file object having the properties:
//url, name, size(formatted), width, height, date(formatted), bytes(integer size in bytes), time(integer date timestamp)
